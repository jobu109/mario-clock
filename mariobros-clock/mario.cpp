...
void Mario::jump() {
  if (_state != JUMPING && (millis() - lastMillis > 500) ) {
    ...
    if ( _x -14 <= goomba.get_x() && goomba._state == RIGHT)
    {
        goomba.setState(Goomba::SMASHED);
    }
...
void Mario::idle() {
  if (_state != IDLE) {
    ...
    _width = MARIO_IDLE_SIZE[0];  // Set width
    _height = MARIO_IDLE_SIZE[1]; // Set height
    _sprite = MARIO_IDLE;         // Set sprite
  }
}
...
void Mario::init() {
  ...
  idle();  // Ensure Mario starts in an idle state
}
